    ...
    def conver_ldap_to_django(self, date):
        ...
        for k in date.keys():
            if k in self.AUTH_LDAP_USER_ATTR_MAP.keys():
                if k == "givenName":
                    slov[self.AUTH_LDAP_USER_ATTR_MAP[k]] = " ".join([i.capitalize() for i in date[k].split(" ") ])
                else:
...
class CheckUserDjango():
    ...
    def groups_a(self,gr_list):
        q = list()
        if gr_list == None:
            gr, flag = CustomGroup.obj.get_or_create(name=DEFAULT_GROUP_FOR_LDAP)
            q.append(gr)
            return q
        ...
        for g in result:
            _,gr = g.split("=")
            gr ,flag = CustomGroup.obj.get_or_create(name=gr)
            q.append(gr)
        return q

...
"""
Source: https://github.com/codekoala/django-watermark/blob/master/watermarker/utils.py
...
def watermark_thumbnail_processor(im, requested_size, opts):
    """Процессор для sorl-thumbnail"""

    from django.conf import settings

    if requested_size[0] > 300 and requested_size[1] > 300:
        if 'x' in opts and opts['x']:
            mark = Image.open(find(settings.WATERMARK), 'r')
            im = watermark(im, mark, 'BR', 1)
            im = im.convert('RGB')
    return im
watermark_thumbnail_processor.valid_options = ['x', ]
...
def determine_scale(scale, img, mark):
    ...
    if scale:
        ...
        if type(scale) in (str, unicode) and scale.lower() == 'f':
            ...
            raise ValueError(
                'Invalid scale value "%s"!  Valid values are 1) "F" for ratio-preserving scaling and 2) floating-point numbers and integers greater than 0.' % (scale,))
